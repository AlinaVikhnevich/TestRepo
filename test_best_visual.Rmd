...
df <- df %>%
  mutate(
    ...
    Code = if_else(str_detect(Code, "^[A-Z]{3}$"), Code, NA_character_)
  ) %>%
  filter(!is.na(Code)) %>%
...
world_geojson$features <- purrr::map(world_geojson$features, function(feat) {
  ...
  if (stringr::str_detect(name_val, regex("N\\.?\\s*Cyprus|Northern Cyprus", ignore_case = TRUE))) {
    props$iso_a3 <- "CYP"
  ...
  if (stringr::str_detect(name_val, regex("Somaliland", ignore_case = TRUE))) {
    props$iso_a3 <- "SOM"
...
world_geojson$features <- purrr::keep(
  ...
  ~{props <- .x$properties; !identical(props$iso_a3, "-99")}
)
...
report_lines <- c(
  ...
  "",
  "Changes applied:",
  "- Forced Kosovo Entity rows to use XKX to match the custom Kosovo polygon.",
  "- Merged Somalia/Somaliland geometries and ensured Somaliland geojson properties use SOM.",
  "- Merged Cyprus/Northern Cyprus geometries and ensured Northern Cyprus geojson properties use CYP.",
  "",
  "Commands run:",
  "- Rscript -e \"rmarkdown::render('test_best_visual.Rmd')\" (failed: Rscript unavailable in container)",
  "- apt-get update -y (failed: proxy returned 403)",
  "- pip install geopandas --quiet (failed: proxy returned 403)",
  "",
  "Evidence:",
