...
map_frames <- lapply(seq_along(years_available), function(i) {
  list(
    name = as.character(years_available[i]),
    data = list(
      list(
        z = map_year_values[[i]]$z,
        customdata = map_year_values[[i]]$customdata
      )
    )
  )
})
...
map_speed_ms <- 140
map_frame_opts <- list(
  frame = list(duration = map_speed_ms, redraw = FALSE),
...
map_frame_opts <- list(
  frame = list(duration = 125, redraw = FALSE),
  transition = list(duration = 0),
  mode = "immediate"
)
...
map_buttons <- list(
  list(
    ...
    args = list(
      ...
      c(map_frame_opts, list(fromcurrent = FALSE))
    )
  ),
  list(
    label = "Pause",
...
map_js <- sprintf(
  "function(el, x) {
    ...
    const setYear = (idx) => {
      ...
      Plotly.restyle(el, { z: [entry.z], customdata: [entry.customdata] }, [0]);
...
income_frame_opts <- list(
  frame = list(duration = 250, redraw = FALSE),
  transition = list(duration = 0),
  mode = "immediate"
)
...
income_buttons <- list(
  ...
  list(
    ...
    args = list(
      NULL,
      c(income_frame_opts, list(fromcurrent = FALSE))
    )
  ),
  list(
    label = "Pause",
...
continent_buttons <- list(
  ...
  list(
    ...
    args = list(
      NULL,
      c(continent_frame_opts, list(fromcurrent = FALSE))
    )
  ),
  list(
    label = "Pause",
