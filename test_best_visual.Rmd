...
fig <- fig %>%
  add_trace(
    ...
    showscale = FALSE,
    marker = list(
      line = list(color = "#4F494A", width = 0.8)
    ),
    hoverinfo = "skip"
  )
fig <- fig %>%
...
# Visual styling note: the adjustments below modify only the HTML-rendered graphics (map, bar, and line visuals) to control their appearance.
fig <- fig %>% layout(
  title = list(
