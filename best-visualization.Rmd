...
steps <- lapply(seq_along(years_available), function(i) {
  ...
  list(
    ...
    args = list(
      list(
        locations = list(year_df$locations),
        z = list(year_df$z),
        text = list(year_df$text),
        customdata = list(year_df$customdata)
      )
    )
  )
})
...
fig_income$x$layout$sliders[[1]]$y <- -0.08
...
fig_cont$x$layout$sliders[[1]]$y <- -0.08
