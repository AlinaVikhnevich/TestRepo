---
title: "DATA 621 Final: Life Expectancy"
...
#map-wrapper {
  ...
  padding-bottom: 120px; /* gives room for the map slider/buttons below */
}
```
# Core Research Question

How has life expectancy evolved across countries over time, and to what extent do countries differ in their baseline levels and long-run trajectories?
...
fig <- fig %>% layout(
  title = list(
    text = "<b>Life Expectancy by Country (1543 - 2023)</b>",
    x = 0.5,
    xanchor = "center",
    font = list(
  ...
  hoverlabel = list(
    bgcolor = "#E0D7C5",        # Hover background color
    bordercolor = "#8A623A",    # Hover border color
    font = list(
      family = "Cormorant Garamond, serif",
      size = 16,
...
fig_income <- plot_ly(
  ...
  marker = list(
    ...
    line = list(color = "rgba(66,46,18,0.55)", width = 0.8)
  ),
...
continents <- c("Oceania", "Europe", "Americas", "Asia", "World", "Africa")
...
continent_js <- sprintf(
  "function(el, x) {
    ...
    const buildSummaryHTML = (idx, yearPaths) => {
      const y = years[idx];
      const lines = [];
      lines.push('<b>Year: ' + y + '</b>');

      yearPaths.forEach((trace, i) => {
        const ys = trace.y || [];
        const name = series[i] || ('Series ' + (i + 1));

        if (!ys.length) return;
        const v = ys[ys.length - 1];
        if (v === null || v === undefined || Number.isNaN(Number(v))) return;

        lines.push(name + ': ' + Number(v).toFixed(1));
      });
    ...
    const setYear = (idx) => {
      ...
      const xYear = years[idx];

      // Update each line to show data up to the current year
      yearPaths.forEach((trace, i) => {
        Plotly.restyle(el, { x: [trace.x], y: [trace.y] }, [i]);
      });

      // Vertical guide line at the current year
      const guideLine = {
        type: 'line',
        xref: 'x',
        yref: 'paper',
      ...
      const summary = {
        ...
        align: 'left',
        showarrow: false,
        text: buildSummaryHTML(idx, yearPaths),
        bgcolor: '#E6D2B8',
        bordercolor: '#694725',
        borderwidth: 1,
        borderpad: 10,
        font: { family: 'Cormorant Garamond, serif', size: 20, color: '#1f2933' }
      };
    ...
    const startIndex = findIndex(payload.selected_year);
    if (startIndex >= 0) setYear(startIndex);
